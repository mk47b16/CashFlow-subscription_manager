...
function Dashboard({ dataset, currentMonthLabel, selectedMonthKey, subscriptions, anomalies, monthlySpend, potentialSavings, learningCount, alertsCount, chartsReady, showAllRecent, onToggleShowAll, onSelect, onNavigate }: { dataset: Transaction[]; currentMonthLabel: string; selectedMonthKey: string; subscriptions: Subscription[]; anomalies: Anomaly[]; monthlySpend: number; potentialSavings: number; learningCount: number; alertsCount: number; chartsReady: boolean; showAllRecent: boolean; onToggleShowAll: () => void; onSelect: (item: Subscription | Anomaly) => void; onNavigate: () => void }) {
  ...
  return <div className="dashboard-page">
    <section className="metric-grid">
      ...
      <Metric icon={<CreditCard/>} label="Recurring commitments" value={String(subscriptions.length)} trend={learningCount > 0 ? `${learningCount} new commitment${learningCount === 1 ? "" : "s"} learning` : "All confirmed"} tone="learning" onClick={onNavigate}/>
      <Metric icon={<AlertTriangle/>} label="Spending anomalies" value={String(anomalies.length)} trend={anomalies.length ? "Needs a quick look" : "Nothing unusual"} tone="risk" onClick={() => anomalies[0] && onSelect(anomalies[0])}/>
    </section>
...
function Metric({ icon, label, value, trend, tone, onClick }: {icon: React.ReactNode; label: string; value: string; trend: string; tone: "good" | "risk" | "learning"; onClick?: () => void}) { return <article className={onClick ? "metric panel clickable" : "metric panel"} onClick={onClick} role={onClick ? "button" : undefined} tabIndex={onClick ? 0 : undefined} onKeyDown={onClick ? (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onClick(); } } : undefined}><span className={`metric-icon ${tone}`}>{icon}</span><p>{label}</p><h2>{value}</h2><small className={tone}>{trend}</small></article>; }

...
import { AlertTriangle, ArrowDownLeft, ArrowLeftRight, ArrowUpRight, Bell, BrainCircuit, CalendarDays, Car, Check, ChevronRight, CircleHelp, CreditCard, DollarSign, Home, Menu, Music, Plus, ScanLine, ShieldCheck, ShoppingBag, Sparkles, Utensils, X, Zap } from "lucide-react";
    ...
    <aside className={`sidebar${sidebarOpen ? " open" : ""}`}>
      ...
      <div className="sidebar-footer">
        <div className="sidebar-status-row">
          ...
          <button className="icon-btn" aria-label={`Notifications, ${alerts.length} unread`} title={`${alerts.length} subscription${alerts.length === 1 ? "" : "s"} to review`} onClick={() => { if (alerts[0]) setSelected(alerts[0]); else navigate("subscriptions"); setSidebarOpen(false); }}><Bell size={18}/>{alerts.length > 0 && <span className="bell-badge" aria-hidden="true">{alerts.length}</span>}</button>
        </div>
...
const categoryIcons: Partial<Record<Transaction["category"], React.ComponentType<{ size?: number }>>> = { Food: Utensils, Transport: Car, Shopping: ShoppingBag, Housing: Home, Entertainment: Music, Transfer: ArrowLeftRight };

function TransactionRow({ transaction }: { transaction: Transaction }) {
  const Icon = categoryIcons[transaction.category] ?? ArrowDownLeft;
  const color = categoryColors[transaction.category as keyof typeof categoryColors];
  return <div className="transaction-row"><span className="merchant-icon" style={{ background: color + "20", color }}><Icon size={16}/></span><span><strong>{transaction.merchant}</strong><small>{transaction.category} · {dateLabel(transaction.date)}</small></span><b>−{currency(transaction.amount)}</b></div>;
}

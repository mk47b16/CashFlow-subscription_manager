  ...
  return <main className="app-shell">
    ...
    <section className="content">
      <header className="topbar">
        ...
        <div className="header-actions"><label className="month-select"><CalendarDays size={16}/><select value={selectedMonthKey} aria-label="Select month" onChange={event => setActiveMonth(event.target.value)}>{availableMonths.slice().reverse().map(month => <option key={month} value={month}>{new Intl.DateTimeFormat("en-IN", { month: "long", year: "numeric" }).format(new Date(`${month}-01T00:00:00`))}</option>)}</select></label><button onClick={runScan} disabled={scanning} className="scan-btn" aria-label={scanning ? "Rescanning transactions" : "Reset dismissed items and rescan"}><ScanLine size={17}/>{scanning ? "Rescanning…" : "Reset & Rescan"}</button></div>
      </header>
...
function ScanProgress({ stage }: { stage: number }) {
  const steps = ["Clearing dismissed items...", "Re-analyzing transactions...", "Updating insights..."];

...
import { Anomaly, asCategory, asKind, Subscription, Transaction } from "../lib/types";
...
export function CashFlowApp() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      const rows: Transaction[] = (data ?? []).map(row => ({ id: row.id, date: row.date, merchant: row.merchant, amount: Number(row.amount), category: asCategory(row.category), kind: asKind(row.kind), engagementDays: row.engagement_days ?? undefined }));
  ...
  const addSubscription = async (entry: SubscriptionEntry) => {
    ...
    const saved: Transaction = { id: data.id, date: data.date, merchant: data.merchant, amount: Number(data.amount), category: asCategory(data.category), kind: asKind(data.kind), engagementDays: data.engagement_days ?? undefined };
...
function ExplainDrawer({ item, classification, onClose, onKeep, setClassification }: { item: Subscription | Anomaly; classification?: string; onClose:()=>void; onKeep:(id:string)=>void; setClassification:(value:string)=>void }) {
  const subscription = "members" in item; const s = subscription ? item : null; const a = subscription ? null : item;

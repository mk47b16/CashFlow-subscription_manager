  ...
  return <main className="app-shell">
    <aside className={`sidebar${sidebarOpen ? " open" : ""}`}>
      ...
      <nav aria-label="Primary navigation">
        ...
        <button className="nav-item" disabled={anomalies.length === 0} aria-label={`Anomaly watch, ${anomalies.length} found`} onClick={() => { if (anomalies[0]) setSelected(anomalies[0]); setSidebarOpen(false); }}><AlertTriangle size={18}/> Anomaly watch <em aria-hidden="true">{anomalies.length}</em></button>
      </nav>
      ...
      <div className="sidebar-footer">
        <div className="sidebar-status-row">
          ...
          <button className="icon-btn" aria-label={`Notifications, ${alerts.length} unread`} title={`${alerts.length} subscription${alerts.length === 1 ? "" : "s"} to review`} onClick={() => { if (alerts[0]) setSelected(alerts[0]); else navigate("subscriptions"); setSidebarOpen(false); }}><Bell size={18}/>{alerts.length > 0 && <i aria-hidden="true"/>}</button>
        </div>
    ...
    <section className="content">
      <header className="topbar">
        <button className="menu-btn" aria-label="Open navigation" aria-expanded={sidebarOpen} onClick={() => setSidebarOpen(o => !o)}><Menu size={20}/>{anomalies.length > 0 && <i aria-hidden="true"/>}</button>

...
export function detectSubscriptions(source = transactions): Subscription[] {
  ...
  source.filter(t => t.kind === "debit").forEach(t => {
    const c = canonicalMerchant(t.merchant); if (["Netflix", "Spotify", "Amazon Prime", "YouTube Premium", "Notion AI", "Arjun Rent Split"].includes(c)) groups.set(c, [...(groups.get(c) ?? []), t]);
  });
